...
`https://cs571api.cs.wisc.edu/rest/su25/hw8/articles` returns a short summary of each of the news articles including an `id`, `img`, `title`, `tags`, and `fullArticleId`. You may assume the `id` is unique for each article. The `fullArticleId` is also unique and can be used to fetch additional details below.

`https://cs571api.cs.wisc.edu/rest/su25/hw8/article?id=ARTICLE_ID` returns the details for a particular article id (from the `fullArticleId` above). **This endpoint is intentionally slow.** These details include all of the properties of the short summary as well as additional `body`, `author`, `posted`, and `url` properties. In particular...
 ...
 The `img` may be appended to `https://raw.githubusercontent.com/CS571-SU25/hw8-api-static-content/main/` to get the requested image, e.g. the image `milkyway.jpg` is hosted at `https://raw.githubusercontent.com/CS571-SU25/hw8-api-static-content/main/milkyway.jpg`
...
Allow the user to navigate between two tabs: a tab for "News" and a tab for "Preferences". I would recommend using a BottomTabNavigator in `BadgerTabs.jsx` to navigate between screens for `BadgerNewsScreen.jsx` and `BadgerPreferencesScreen.jsx`. **You must use React Navigation.**

**Note:** While tab bar icons are *not* required, you can consider adding them by using icons from the `@expo/vector-icons` library within the `tabBarIcon` screen option.

![](_figures/Step1.png)

### 2. Display News

Fetch the short news article summaries from `https://cs571api.cs.wisc.edu/rest/su25/hw8/articles` and display them to the screen as a card, including their image and title text. I'd recommend creating a component such as `BadgerNewsItemCard` to display each short summary as a card. Use the article's `id` as the unique key.
